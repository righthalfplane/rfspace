...
sdr.writeSetting("direct_samp","2")
...
fpwd= open("0rx_am_760000_2000000_fc.raw",'wb')
...
def receive():
    ...
    while True:
        ...
        #fpwd.write(buff2)
        #print(buff2[0]," ",sr.ret)
        #print("0 ",{nl},"0 ",{nc}," ret ",sr.ret," ",len(buff2))
        if sr.ret != nc:
            if sr.ret <= 0:
                #print("-1 return")
                break
            #print("1 ",{nl}," 1 ",{nl+sr.ret}," ret ",sr.ret)
...
try:
    ...
    def audio_callback(outdata, frames, time2, status):
        ...
        dec=int(sampleRate/audioRate);
        
        audio_data=buff1*freq_shift;
        
        fpwd.write(audio_data.astype(np.complex64))
